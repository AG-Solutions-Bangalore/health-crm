...
const SessionTimeoutTracker = ({ expiryTime, onLogout }) => {
  ...
  const resetInactivityTimer = () => {
    ...
    inactivityTimer.current = setTimeout(checkInactivity, 7 * 1000);
  };
  ...
  // mouse postion handling from the hooks -- refer useMouse position
  useEffect(() => {
    if (!isInitialized.current || !isTokenPresent()) return;
  ...
  const handleVisibilityChange = () => {
    ...
    if (isNowActive) {
      ...
      if (timeSinceLastActivity >= 7 * 1000) { 
        console.log("Inactive while tab was hidden, logging out");
        performLogout();
      } else {
      
        console.log("Tab became active, resetting timer");
        resetInactivityTimer();
      }
    }
